...
import { AddEditUserData, AddEditUserResponse } from './add-edit-user/add-edit-user.model';
import {
  MOCK_USERS,
  USER_SERVICE_CONFIG,
  USER_VALIDATION
...
export class UserService {
  ...
  addUser(userData: AddEditUserData): Observable<AddEditUserResponse> {
    ...
    if (emailExists) {
      return throwError(() => USER_SERVICE_CONFIG.ERROR_MESSAGES.EMAIL_EXISTS);
    }

    const newUser: User = {
      id: Math.max(...this.MOCK_USERS.map(u => u.id)) + USER_VALIDATION.MIN_ID,
    ...
    return of({
      success: true,
      user: newUser,
      message: USER_SERVICE_CONFIG.SUCCESS_MESSAGES.CREATE_USER
    });
  }
  ...
  updateUser(userId: number, userData: AddEditUserData): Observable<AddEditUserResponse> {
    ...
    if (userIndex === -1) {
      return throwError(() => USER_SERVICE_CONFIG.ERROR_MESSAGES.USER_NOT_FOUND);
    }
    ...
    return of({
      success: true,
      user: updatedUser,
      message: USER_SERVICE_CONFIG.SUCCESS_MESSAGES.UPDATE_USER
    });
  }
